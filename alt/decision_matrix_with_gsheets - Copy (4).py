...
for opt in options:
    ...
    with st.container():
        ...
        # Bildbreite pro Option (als Textfeld ohne +/-)
        default_width = 250
        width_input = st.text_input(
            f"📏 Image width for {label} (in pixels, 100–400)",
            value=str(default_width),
            key=f"img_width_{opt}"
        )

        try:
            image_width_opt = int(width_input)
            if not (100 <= image_width_opt <= 400):
                st.warning("⚠️ Please enter a value between 100 and 400.")
                image_width_opt = default_width
        except ValueError:
        ...
        if existing_links:
            ...
            cols_existing = st.columns(min(4, len(existing_links)))
            for idx, url in enumerate(existing_links):
                with cols_existing[idx % len(cols_existing)]:
                    if "drive.google.com" in url and "id=" in url:
                        file_id = url.split("id=")[-1].strip()
                        embed_url = f"https://drive.google.com/file/d/{file_id}/preview"
                        st.components.v1.iframe(embed_url, height=280)
                    else:
        ...
        #                 st.image(direct_url, width=image_width_opt)
        #             else:
        #                 st.warning("⚠️ Invalid image URL.")
        ...
        #     for idx, url in enumerate(existing_links):
        #         with cols_existing[idx % len(cols_existing)]:
        #             st.image(url, width=image_width_opt)
...
#         # Bildhöhe pro Option
#         image_height_opt = st.number_input(
#             f"📏 Image height for {label} (in pixels)",
#             min_value=100, max_value=400, value=180, step=10,
#             key=f"img_height_{opt}"
#         )
...
#         )
#         urls = []
#         if existing_urls.get(opt):
#             urls.extend(existing_urls[opt].split(", "))
...
#                     urls.append(link)
#                     with image_cols[idx % len(image_cols)]:
#                         st.image(file, height=image_height_opt)
#             except Exception as e:
#                 st.warning(f"❌ An unexpected error occurred during image processing: {e}")
...
#         urls = []
#         # Add existing URLs to the list if they exist, to prevent overwriting
#         if existing_urls.get(opt):
#             urls.extend(existing_urls[opt].split(", "))
...
#         # Ensure unique URLs and store them back
#         image_urls[opt] = ", ".join(sorted(list(set(urls))))
#         # --- End Moved Image Upload Section ---
